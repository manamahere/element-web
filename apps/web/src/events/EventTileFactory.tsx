...
import { CallStartedTileViewModel } from "../viewmodels/room/timeline/event-tile/CallStartedTileViewModel";
...
function CallStartedTilViewWrapped({ mxEvent }: IBodyProps): JSX.Element {
    const vm = useCreateAutoDisposedViewModel(() => new CallStartedTileViewModel({ mxEvent }));
    return <CallStartedTileView vm={vm} />;
}

export const ElementCallStartedFactory: Factory = (ref, props) => {
    return <CallStartedTilViewWrapped {...props} />;
};
...
const EVENT_TILE_TYPES = new Map<string, Factory>([
    ...
    [EventType.RTCNotification, ElementCallStartedFactory],
]);
